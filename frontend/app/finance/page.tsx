      ...
      <div className={`module-with-menu-shell finance-shell ${sidebarOpen ? "" : "nav-collapsed"}`}>
        ...
        <main className="finance-workspace">
          <header className="finance-header">
            ...
            <div className="finance-header-actions"><label className="finance-search"><span className="finance-search-label">Buscar</span><input aria-label="Buscar en Finance OS" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cliente, factura o movimiento" /></label><div className="finance-notification-wrap"><button className="finance-bell" type="button" onClick={() => setNotificationsOpen((value) => !value)} aria-label="Ver notificaciones">{overview?.exceptions.open || overview?.invoices.overdue ? "!" : "o"}</button>{notificationsOpen ? <div className="finance-notification-panel"><strong>Notificaciones</strong>{overview?.invoices.overdue ? <span>{overview.invoices.overdue} facturas vencidas requieren atencion.</span> : null}{overview?.exceptions.open ? <span>{overview.exceptions.open} excepciones estan pendientes de revision.</span> : null}{!overview?.invoices.overdue && !overview?.exceptions.open ? <span>No hay alertas financieras nuevas.</span> : null}</div> : null}</div><AccountPill fallbackName={agent?.name || "Usuario"} /></div>
          </header>
