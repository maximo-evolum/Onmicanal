...
function bankMovementLabel(data: Record<string, unknown>) {
  const direction = String(data.direction || "").toUpperCase();
  if (direction === "DEBIT" || String(data.movementType || "").toUpperCase() === "CARGO") return { label: "Cargo", className: "debit", detail: "Egreso bancario" };
  if (direction === "CREDIT" || String(data.movementType || "").toUpperCase() === "ABONO") return { label: "Abono", className: "credit", detail: "Ingreso bancario" };
  return { label: "Por revisar", className: "review", detail: "Tipo no identificado" };
}
...
function FinanceTable({ records, kind, query = "" }: { records: IndustryRecord[]; kind: "invoice" | "movement" | "exception" | "collection"; query?: string }) {
  ...
  return <div className="finance-table">{visibleRecords.map((record) => { const data = asData(record); const movement = kind === "movement" ? bankMovementLabel(data) : null; return <div key={record.id} className={movement ? `finance-movement-row ${movement.className}` : ""}><div><strong>{financeLabel(record.title)}</strong>{kind === "movement" && movement ? <span><b className={`finance-movement-type ${movement.className}`}>{movement.label}</b> · {shortDate(data.transactionDate || data.date)} · {financeLabel(data.reference, "Sin referencia")}<small>{movement.detail}{data.directionSource ? ` · identificado por ${financeLabel(data.directionSource)}` : ""}</small></span> : <span>{kind === "invoice" ? `${financeLabel(data.clientName)} · vence ${shortDate(data.dueDate)}` : financeLabel(data.type, financeLabel(data.detail, "Sin detalle"))}</span>}</div><b className={movement ? `finance-movement-amount ${movement.className}` : ""}>{kind === "invoice" || kind === "movement" ? money(data.amount) : financeLabel(record.status)}</b></div>; })}</div>;
}
