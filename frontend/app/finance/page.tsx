      ...
      <div className={`module-with-menu-shell finance-shell ${sidebarOpen ? "" : "nav-collapsed"}`}>
        ...
        <main className="finance-workspace">
          <header className="finance-header">
            <div><span>EVOLUM FINANCE OS</span><h1>Cuenta por cobrar, bajo control.</h1><p>{active.detail}</p></div>
...
function FinanceTable({ records, kind }: { records: IndustryRecord[]; kind: "invoice" | "movement" | "exception" | "collection" }) {
  if (!records.length) return <p className="finance-empty">Aun no hay registros en esta seccion.</p>;
  return <div className="finance-table">{records.map((record) => { const data = asData(record); return <div key={record.id}><div><strong>{record.title}</strong><span>{kind === "invoice" ? `${text(data.clientName)} · vence ${shortDate(data.dueDate)}` : kind === "movement" ? `${shortDate(data.date)} · ${text(data.reference, "Sin referencia")}` : text(data.type, text(data.detail, "Sin detalle"))}</span></div><b>{kind === "invoice" || kind === "movement" ? money(data.amount) : text(record.status)}</b></div>; })}</div>;
}
