    ...
    <ModuleGate moduleKey="finance_analytics">
      <div className={`module-with-menu-shell product-workspace finance-shell finance-product-workspace ${sidebarOpen ? "" : "nav-collapsed"}`}>
        ...
        <main className="finance-workspace">
          ...
          {activeTab === "conciliacion" ? <section className="finance-card"><h2>Sugerencias de conciliación</h2><p>La IA solo propone abonos externos: excluye cargos, comisiones y traspasos internos. Toda confirmación queda registrada con evidencia.</p><div className="finance-suggestions">{suggestions.map((item) => <article key={`${item.movement.id}-${item.invoice.id}`}><div><b className={`finance-confidence ${item.level.toLowerCase()}`}>{item.confidence}% {financeConfidenceLabel(item.level)}</b><strong>{financeLabel(asData(item.movement).description, financeLabel(item.movement.title))}</strong><span>{money(asData(item.movement).amount)} · {shortDate(asData(item.movement).transactionDate || asData(item.movement).date)}</span></div><div><strong>{item.grouped ? `${item.invoices?.length || item.invoiceIds?.length || 2} documentos cubiertos por un pago` : financeLabel(asData(item.invoice).invoiceNumber, financeLabel(item.invoice.title))}</strong><span>{item.grouped ? (item.invoices || []).map((invoice) => financeLabel(asData(invoice).invoiceNumber, financeLabel(invoice.title))).join(" · ") : `${financeLabel(asData(item.invoice).clientName || asData(item.invoice).customerName)} · ${money(asData(item.invoice).amount)}`}</span><small>{financeReasons(item.reasons)}</small></div><button className="primary-btn" type="button" disabled={saving} onClick={() => approveSuggestion(item)}>{item.grouped ? "Confirmar grupo" : "Confirmar"}</button></article>)}{!suggestions.length && !loading ? <p className="finance-empty">Aún no hay coincidencias. Carga facturas y movimientos para calcularlas.</p> : null}</div></section> : null}
